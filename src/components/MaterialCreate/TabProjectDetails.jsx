...
const TabProjectDetails = ({ formData, onUpdate, errors, setError }) => {
    ...
    const fetchClients = async (type, companyId) => {
        if (!type || !companyId) {
            setClientFetchError('Client type or company ID is missing');
            return;
        }
        ...
        try {
            ...
            if (!token) {
                setClientFetchError('Authentication token not found. Please login again.');
                return;
            }
            ...
            if (result?.status && Array.isArray(result.data)) {
                ...
                if (options.length === 1 && !formData.selectedClient) {
                    handleClientSelect(options[0]);
                }
            } else {
                setClientFetchError(result?.message || 'No clients available for the selected type');
                setClients([]);
            }
        } catch (error) {
            console.error('Error fetching clients:', error);
            setClientFetchError('Failed to load clients. Please try again later.');
            setClients([]);
        } finally {
    ...
    useEffect(() => {
        ...
        if (!formData.selectedMonth) {
            ...
            onUpdate('selectedMonth', defaultMonth);
        }
        ...
        if (clientType && companyId) {
            ...
            setClientFetchError(!clientType ? 'Please select a project type first' : 'Company not selected');
        }
    ...
    const handleMonthChange = (e) => {
        ...
        if (setError) {
            setError(prev => ({ ...prev, step1Month: null }));
        }
    };
    ...
    const handleYearChange = (e) => {
        ...
        if (setError) {
            setError(prev => ({ ...prev, step1Year: null }));
        }
    };
            ...
            <form id="project-details">
                <fieldset>
                    <div className="mb-5">
                        ...
                        {(errorMessages.length > 0 || clientFetchError) && (
                            <div className="alert alert-danger mt-2 py-2 px-3">
                                ...
                                <ul className="mb-0 ps-3">
                                    ...
                                    {clientFetchError && !errors.step1 && <li>{clientFetchError}</li>}
                                </ul>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="row mb-5 pb-5">
                            ...
                            <div className="col-lg-3 mb-4">
                                ...
                                <SelectDropdown
                                    ...
                                    error={!!errors.step1 || !!clientFetchError}
                                    isLoading={loadingClients}
                                    loadingText="Loading clients..."
                                    disabled={!clientType || loadingClients || !!clientFetchError}
                                />
                                {loadingClients && (
                                    <small className="text-muted">Fetching clients from API...</small>
                                )}
                                {clientFetchError && !loadingClients && (
                                    <small className="text-danger">{clientFetchError}</small>
                                )}
                                {errors.step1 && (
                                    <small className="text-danger">{errors.step1}</small>
                                )}
                                {!clientFetchError && !loadingClients && clients.length === 0 && (
                                    <small className="text-muted">No clients found for the selected project type</small>
                                )}
                            </div>
